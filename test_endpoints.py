...
def test_optimize():
    ...
    # verify greedy score/cost ordering was respected
    # score = (lst*0.7) - (ndvi*0.3); best ratio should be first
    def ratio(p):
...
def test_esg(selected: list):
    ...
    print("esg-report:", r)
    n = len(selected)
    assert r["trees_planted"] == n
    assert r["carbon_10yr"] == round(n * 21.77, 2)
    assert r["temp_reduction"] == round(n * 0.02, 3)
